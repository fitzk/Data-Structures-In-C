...
struct BSTree*  newBSTree()
{
	...
	printf("%s\n", "allocating memory (newBSTree)");
	struct BSTree *tree = (struct BSTree *)malloc(sizeof(struct BSTree));
	printf("%s%d\n", "ADDRESS OF TREE: ", tree);
	assert(tree != 0);
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
  ...
  while (compare(temp->val,val) != 0) {
    ...
    // if there’s nowhere else to go and value != d per while loop, value doesn’t exist
		if (temp == NULL) {
			printf("%s\n", "terminated Contains");
    ...
    // if value is found above, while loop stops executing and drops down to next statement
		return 1;
}
